...
from tff_utils import log_attn_and_fc_layers, calculate_val_metrics, evaluate, \
    _server_optimizer_fn, _client_optimizer_fn, get_model_with_new_last_layer, modify_model_for_lower_bound_loop, \
    calculate_mean_f1_diff, get_permissable_attn_layers, modify_model_for_AALF_loop, \
    check_perm_trainable_status
...
def calculate_metrics(train_metrics, mode='train'):
    ...
    acc = train_metrics[mode][tag + 'binary_accuracy']
...
client_id_colname = 'client_id'
src = 'Datasets/'
df1 = pd.read_csv(src + 'train_ptb_preproc_sigma_0.csv')  # read data
df1[client_id_colname] = 1
df2 = pd.read_csv(src + 'train_ptb_preproc_sigma_0.02.csv')
df2[client_id_colname] = 2
df3 = pd.read_csv(src + 'train_ptb_preproc_sigma_0.05.csv')
...
train_writer = tf.summary.create_file_writer("FL_logs/train/PTB_aalf/")
val_writer = tf.summary.create_file_writer("FL_logs/val/PTB_aalf/")

# create val dataset ###
df_val = pd.read_csv(src + 'val_ptb_preproc_sigma_0.csv')
val_dataset = dataframe_to_dataset(df_val, label_col='label')
val_dataset = val_dataset.batch(1)


# initiate certain important variables
prev_f1_arr = np.zeros(2)
mean_f1_diff, mean_curr_f1, mean_prev_f1 = 0.0, 0.0, 0.0
all_attn_layer_ix_list = [6, 7, 18, 19, 30, 31, 42, 43]
curr_permissable_attn_layer_ix_list = []
curr_permissable_frozen_attn_layer_ix_list = []
curr_permissable_frozen_period_dict = {6: 0, 7: 0, 18: 0, 19: 0, 30: 0, 31: 0, 42: 0, 43: 0}
permissable_trainable_status_dict = {6: False, 7: False, 18: False, 19: False, 30: False, 31: False, 42: False, 43: False}
F1_DIFF_THRESH = 0.03
MAX_FROZEN_PERIOD = 1
MIN_VALUE_DIFF = 0.03
MIN_COVERAGE = 0.2  # Min 20% of the parameters in a layer should have high changes for that layer to be trainable.
FIRST_EPOCH = True
# training loop ###
for i in range(0, 100, 4):
    if FIRST_EPOCH:
        model = tf.keras.models.load_model(save_model_dir + "whole_attn_model.h5", compile=False)
        keras_model = get_model_with_new_last_layer(pretrained_model=model, num_classes=2)
    else:
        keras_model.load_weights("saved_models/FL/model_ptb_aalf.h5")

    if FIRST_EPOCH:
        keras_model = modify_model_for_lower_bound_loop(model=keras_model)
        curr_permissable_attn_layer_ix_list.append(all_attn_layer_ix_list[-1])
        permissable_trainable_status_dict[all_attn_layer_ix_list[-1]] = True
        # save as prev model
        prev_model = tf.keras.models.clone_model(keras_model)
        FIRST_EPOCH = False
    else:
        curr_permissable_list = get_permissable_attn_layers(mean_f1_diff=mean_f1_diff,
                                                            mean_curr_f1=mean_curr_f1,
                                                            mean_prev_f1=mean_prev_f1,
                                                            curr_permissable_list=curr_permissable_attn_layer_ix_list,
                                                            all_attn_list=all_attn_layer_ix_list,
                                                            f1_diff_thresh=F1_DIFF_THRESH)
        # old model-> prev_model, curr model -> keras model
        # get coverage and determine frozen
        keras_model, curr_permissable_frozen_period_dict, permissable_trainable_status_dict = check_perm_trainable_status(
            curr_model=keras_model,
            prev_model=prev_model,
            curr_permissable_list=curr_permissable_attn_layer_ix_list,
            perm_layer_frozen_period=curr_permissable_frozen_period_dict,
            perm_status_dict=permissable_trainable_status_dict,
            min_val=MIN_VALUE_DIFF,
            min_coverage=MIN_COVERAGE)
        # modify model with final decisions on trainable status
        keras_model = modify_model_for_AALF_loop(model=keras_model, perm_status_dict=permissable_trainable_status_dict)
        # save as prev model
        prev_model = tf.keras.models.clone_model(keras_model)
    ...
    for round_num in range(4):  # num_epochs
        ...
        if epoch % 1 == 0 or epoch == 4 - 1:
            print(f'i {i}, Round {round_num}')
            server_state.model.assign_weights_to(eval_model)
            eval_model.save_weights("saved_models/FL/model_ptb_aalf.h5")
            ...
            mean_f1_diff, mean_curr_f1, mean_prev_f1, prev_f1_arr = calculate_mean_f1_diff(val_f1_list,
                                                                                           prev_f1_arr=prev_f1_arr)
            print(f'Validation mean_curr_f1: {mean_curr_f1} anf f1 list: {val_f1_list}.')
            # Logging
            ...
            with val_writer.as_default():  # to tensorboard for val
                ...
                log_scalar_metrics(loss=val_loss,
                                   ...
                                   time=0,
                                   epoch=epoch)
                tf.summary.scalar("mean_f1_diff", mean_f1_diff, step=epoch)
