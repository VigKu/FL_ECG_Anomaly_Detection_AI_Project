...
from tff_utils import log_attn_and_fc_layers, calculate_val_metrics, evaluate, \
    ...
    modify_model_for_lower_bound_loop, calculate_mean_f1_diff, get_permissable_attn_layers, modify_model_for_AALF_loop, \
...
train_writer = tf.summary.create_file_writer("FL_logs/train/ecg5000_aalf/")
val_writer = tf.summary.create_file_writer("FL_logs/val/ecg5000_aalf/")
...
for i in range(0, 100, 4):
    if FIRST_EPOCH:
        ...
        keras_model.load_weights("saved_models/FL/model_ecg5000_aalf.h5")
    ...
    if FIRST_EPOCH:
        ...
        keras_model, curr_permissable_frozen_period_dict, permissable_trainable_status_dict = check_perm_trainable_status(curr_model=keras_model,
    ...
    for round_num in range(4):  # num_epochs
        ...
        if epoch % 1 == 0 or round_num == 4 - 1:
            print(f'i {i}, Round {round_num}')
            server_state.model.assign_weights_to(eval_model)
            eval_model.save_weights("saved_models/FL/model_ecg5000_aalf.h5")
            ...
            print(f'Validation mean_curr_f1: {mean_curr_f1}.')
            # Logging
